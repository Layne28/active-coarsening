...
def main():
    ...
    if do_random_initial_velocity==1:
        p = np.sqrt(D0/tau)*rng.standard_normal(size=3)#np.random.normal(loc=0.0, scale=np.sqrt(D0/tau), size=3)
    ...
    for n in range(nsteps):
        ...
        p += -(dt/tau)*p + (np.sqrt(2*D0*dt)/tau)*rng.standard_normal(size=3) #update self-propulsion
        disp[n,:] = r-pos[n,:]
